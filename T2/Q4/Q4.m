...
estado_inicial = [0 0 0 0 0 0 1]; %igual ao tamanho de m, estado inicial qualquer
h =  [2 -0.5 0.5]; %canal
SNR = 10;
x = randint(1,Nb,2);
...
taps = objetoLFSR.rand_vetor(1);
%geracao de codigo, lembrando que o codigo é ciclico
codigo = (objetoLFSR.lfsr(estado_inicial,taps,m))*2 - 1;
...
Eb = sum(s_hat.^2)/Nb;
for ruido = 1:SNR
    %gerar ruido linearmente, pois nao estamos tratando com sinais no
    %dominio t
